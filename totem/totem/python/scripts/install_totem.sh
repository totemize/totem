...
if [ "$SETUP_NVME" = true ]; then
    chmod +x "$SCRIPTS_DIR/setup_nvme_mount.sh"
    chmod +x "$SCRIPTS_DIR/mount_nvme_boot.sh"
fi

# Check for NVMe if not explicitly enabled/disabled
if [ "$SETUP_NVME" = false ] && [ "$SKIP_CONFIRM" = false ]; then
    # Check if NVMe devices are present
    if lsblk | grep -q nvme; then
        echo -e "${YELLOW}NVMe device(s) detected. Would you like to set up automatic NVMe mounting? (y/N)${NC}"
        read -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            SETUP_NVME=true
...
if [ "$SETUP_NVME" = true ]; then
    echo "4. NVMe drive auto-mount setup"
fi
...
# Step 4 (Optional): NVMe mounting setup
if [ "$SETUP_NVME" = true ]; then
    total_steps=4
    echo -e "\n${GREEN}Step 4/$total_steps: Setting up NVMe drive auto-mounting...${NC}"
    "$SCRIPTS_DIR/setup_nvme_mount.sh"
    if [ $? -eq 0 ]; then
        echo -e "${GREEN}✓ NVMe drive mounting setup completed successfully${NC}"
    else
        echo -e "${RED}✗ NVMe drive mounting setup failed${NC}"
    fi
fi
...
if [ "$SETUP_NVME" = true ]; then
    echo -e "\nNVMe Drive:"
    echo -e "  Mount Point: ${YELLOW}/mnt/nvme${NC}"
    echo -e "  Auto-mounted at boot: ${GREEN}Yes${NC}"
fi
